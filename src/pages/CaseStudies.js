...
import {
    ...
    FiHeart,         // Healthcare
    FiTruck,         // Logistics
    FiZap,           // Energy (like QCells)
    FiFilm,          // Entertainment
    FiArrowRight,
...
const caseStudiesData = [
    ...
    // Energy
    {
        id: 'energy-qcells-vpp', // Link to existing detailed page
        industry: 'Energy',
        icon: FiZap,
        clientName: 'QCells',
        title: 'Launching a Virtual Power Plant (VPP)',
        challenge: 'Needed to rapidly deploy a scalable platform to manage distributed energy resources (DERs) for grid services.',
        solutionHighlight: 'Leveraged LoyalShift\'s configurable VPP & DERMS Orchestration Platform.',
        result: 'Operational VPP in 6 Months',
        resultIcon: FiCheckCircle,
        link: '/case-studies/qcells', // Specific link
    },
    // {
...
const CaseStudyCard = ({ study }) => (
    ...
    >
        <h3 className={`text-xl font-semibold ${colors.textHeading} mb-2 group-hover:${colors.textPrimary} transition-colors`}>
            {study.title}
        </h3>
        <p className={`text-sm font-medium ${colors.textPrimary} mb-3`}>Client: {study.clientName}</p>
        <p className={`text-base font-semibold ${colors.accentSuccess} mb-4 mt-auto pt-3 flex items-center`}>
        ...
        >
            Read Full Study <FiArrowRight className="ml-1 w-4 h-4 group-hover:translate-x-1 transition-transform" />
        </Link>
    </motion.div>
);
        ...
        <div className={`${colors.bgBase} py-16 md:py-24 overflow-x-hidden`}>
            <div className="container mx-auto px-4">
                ...
                <motion.section
                    ...
                    variants={fadeInUp}
                >
                     <motion.div variants={fadeInUp} className="inline-block p-3 bg-primary-main/10 rounded-full mb-4">
                         <FiBriefcase className={`w-12 h-12 ${colors.textPrimary}`} />
                    </motion.div>
                    <h1 className={`text-4xl md:text-5xl font-bold ${colors.textHeading} mb-4`}>
                        Real Results, Real Transformation
                    </h1>
                    <p className={`text-lg md:text-xl ${colors.textBody} max-w-3xl mx-auto`}>
                        Explore how businesses across diverse sectors leverage LoyalShift to overcome legacy challenges, automate critical processes, and achieve significant, measurable improvements.
                    </p>
                </motion.section>
                ...
                >
                    {Object.entries(studiesByIndustry).map(([industry, { icon: Icon, studies }]) => (
                        <motion.section key={industry} variants={fadeInUp}>
                            {/* --- UPDATED TITLE CONTAINER --- */}
                            {/* Reduced bottom margin (mb) and padding (pb) */}
                            <div className={`flex items-center mb-4 md:mb-6 border-b border-neutral-main/20 pb-2`}>
                                <Icon className={`w-8 h-8 md:w-10 md:h-10 ${colors.textPrimary} mr-4 flex-shrink-0`} />
                                {/* Adjusted title size for potential wrapping/consistency */}
                                <h2 className={`text-3xl md:text-4xl font-bold ${colors.textHeading}`}>{industry}</h2>
                            </div>
                            ...
                            <div className="flex flex-wrap justify-center gap-6 md:gap-8">
                                {studies.map(study => (
                                     // Card width wrapper
                                    <div key={study.id} className="w-full sm:w-[calc(50%-0.75rem)] md:w-[calc(50%-1rem)] lg:w-[calc(33.33%-1rem)] xl:w-[calc(33.33%-1.34rem)]">
                ...
                >
                    ...
                    <p className={`text-lg ${colors.textBody} max-w-3xl mx-auto mb-8`}>
                        Your legacy systems hold untapped potential. Let's discuss how LoyalShift's AI-powered modernization can deliver results tailored to your specific industry and challenges.
                    </p>
