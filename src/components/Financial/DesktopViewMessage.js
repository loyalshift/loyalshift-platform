// src/components/Financial/DesktopViewMessage.js
// Component displayed on mobile devices advising desktop use for the FINANCIAL section.
...
const DesktopViewMessage = () => {
  return (
    <div
      className={`min-h-screen ${colors.background} flex flex-col items-center justify-center text-center p-6`}
    >
      <div
        className={`max-w-md w-full ${colors.surface} p-8 rounded-xl shadow-lg border ${colors.border}`}
      >
        {/* Logo */}
        <img
        ...
        {/* Icon */}
        <FiMonitor
          className={`w-16 h-16 ${colors.primary} mx-auto mb-4 opacity-80`}
        />
        ...
        <p className={`${colors.secondary} text-base leading-relaxed`}>
          Para una mejor visualización y acceso a todas las funcionalidades del
          portal de agentes ANACO Conecta, por favor acceda desde una
