// src/components/Anaco/FloatingItemsBackground.js
...
// SVG Import (Ensure path is correct and your setup supports it)
// If using CRA v4/SVGR: import { ReactComponent as AnacoLogo } from "../../images/anaco.svg";
// If using CRA v5+ standard:
import anacoLogoUrl from "../../images/anaco.svg"; // Adjust path as needed
...
const FloatingItemsBackground = () => {
    ...
    useEffect(() => {
        const generatedDots = Array.from({ length: NUM_DOTS }, (_, i) => ({ id: `dot-${i}`, ...generateItemParams(MIN_DOT_SIZE, MAX_DOT_SIZE) }));
        const generatedLogos = Array.from({ length: NUM_LOGOS }, (_, i) => ({ id: `logo-${i}`, ...generateItemParams(MIN_LOGO_SIZE, MAX_LOGO_SIZE) }));
            ...
            {dots.map(dot => (
                <motion.div
                    ...
                    style={{
                        ...
                        opacity: dot.opacity, backgroundColor: colors.dotColor,
                    }}
                    animate={{
                        x: [`0%`, `${dot.moveX}%`, `0%`],
                        y: [`0%`, `${dot.moveY}%`, `0%`],
                    ...
                    transition={{
                        duration: dot.duration, delay: dot.delay,
